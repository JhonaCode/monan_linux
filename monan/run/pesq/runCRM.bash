...
##cd ../SUBMIT
##rm -r goamazon2_015_km_${LABELI}_${LABELF} 
...
do
	...
	LABELI=${current}
        ...
        LABELF=$(date -d "${formatted_date} + ${RUNHOUR} hours" '+%Y%m%d%H')

	echo "${LABELI}",${LABELF} 

        #current=$(date -d "${formatted_date} + ${RUNHOURSTEP} hour" "+%Y%m%d%H")
        #current=$(date -d "${formatted_date} + 1 day" "+%Y%m%d%H")
        current=$(date -d "${formatted_date} + ${RUNHOURSTEP} hours" '+%Y%m%d%H')

	for EXP_RES in  15 #3 7 10 15 
	#for EXP_RES in  10   ###3 7 10 15 
	do
